...
appdata hull( InputPatch<appdata, 3> ip, uint cpid : SV_OutputControlPointID, uint pid : SV_PrimitiveID )
{
	return ip[cpid];
}
...
[UNITY_domain("tri")]
GS_INPUT domain( HS_CONSTANT_OUTPUT input, float3 tLoc : SV_DomainLocation, const OutputPatch<tess_appdata, 3> patch )
{
	GS_INPUT output;
